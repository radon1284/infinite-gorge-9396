class DashboardController < ApplicationController
  ...
  def reports
    ...
    # @credits = TaskLog.joins(:staff).joins(:client).joins(:user).joins(:employment).select("staffs.full_name AS staff_names").select("staffs.position AS positions").select("clients.full_name AS client_names").select("clients.*, SUM(task_logs.total_hrs) AS today").select("task_logs.multiplier AS task_multiplier").where.not(completed_at: nil).group("staffs.id, users.id, clients.id, employments.id")
