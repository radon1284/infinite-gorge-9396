class DashboardController < ApplicationController
  ...
  def reports
    ...
    @time_today = TaskLog.where('created_at >= ? and created_at <= ?', @dates.beginning_of_day, @dates.end_of_day).where.not(completed_at: nil).sum('total_hrs')
    @by_day = ("%.2f" % @time_today).to_s.split(".").map { |s| s.to_i }
    @total_today = @by_day[0].to_s + ":" + ((@by_day[1]*60)/100).to_s + " Hrs."


    @time_week = TaskLog.where('created_at >= ? and created_at <= ?', @dates.beginning_of_week(start_day = :monday), @dates.end_of_week(end_day = :sunday)).where.not(completed_at: nil).sum('total_hrs')
    @by_week = ("%.2f" % @time_week).to_s.split(".").map { |s| s.to_i }
    @total_week = @by_week[0].to_s + ":" + ((@by_week[1]*60)/100).to_s + " Hrs."


    @time_month = TaskLog.where('created_at >= ? and created_at <= ?', @dates.beginning_of_month, @dates.end_of_month).where.not(completed_at: nil).sum('total_hrs')
    @by_month = ("%.2f" % @time_month).to_s.split(".").map { |s| s.to_i }
    @total_month = @by_month[0].to_s + ":" + ((@by_month[1]*60)/100).to_s + " Hrs."
    ...
    @credits = TaskLog.joins(:staff).joins(:client).joins(:user).joins(:employment).select("staffs.full_name AS staff_names").select("staffs.position AS positions").select("clients.full_name AS client_names").select("employments.*, SUM(employments.multiplier) AS multipliers").select("clients.*, SUM(task_logs.total_hrs) AS today").where.not(completed_at: nil).group("staffs.id, users.id, clients.id employments.id")
