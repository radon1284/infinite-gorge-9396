class DashboardController < ApplicationController
  ...
  def index
    ...
    if  current_user.role == 'client'
      @hrs_staff_by_client = TaskLog.joins(:staff).joins(:client).joins(:user).select("staffs.full_name AS full_names").select("staffs.position AS positions").select("users.email AS emails").select("clients.*, SUM(task_logs.total_hrs) AS today").group("staffs.id, users.id").where(client_id: @client.id)
    end
